...
def generate_images(
    ...
    for seed_idx1, seed1 in enumerate(seeds):
        label = torch.zeros([1, G.c_dim], device=device)
        label[:, class_label] = 1
        os.makedirs(outdir+'/{}'.format(class_label), exist_ok=True)
        row_images = []
        for seed_idx2, seed2 in enumerate(seeds):
            sys.stderr.write('\rGenerating image of class %d for seed1 %d seed2 %d (%d/%d) ...' % (class_label, seed1, seed2, seed_idx2, len(seeds)))
            z_c = torch.from_numpy(np.random.RandomState(seed1).randn(1, G.z_dim//2)).to(device)
            z_s = torch.from_numpy(np.random.RandomState(seed2).randn(1, G.z_dim//2)).to(device)
