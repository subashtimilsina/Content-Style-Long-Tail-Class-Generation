...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    if os.path.isfile(meta_fname):
        ...
        # Generate labels based on the number of images in each directory
        images = ['/'.join(input_img.split('/')[-2:]) for input_img in input_images]
        dirs = [img.split('/')[0] for img in images]
        
        # Count the number of images in each directory
        dir_counts = {dir_name: dirs.count(dir_name) for dir_name in set(dirs)}
        
        # Sort directories by the number of images (descending) and assign labels
        sorted_dirs = sorted(dir_counts.items(), key=lambda x: x[1], reverse=True)
        dir2label = {dir_name: idx for idx, (dir_name, _) in enumerate(sorted_dirs)}
